...
- [x] 1.1 `scripts/seed-bots.ts` 파일 생성. `@ai-jakdang/database`에서 `getDb`, `closeDb`를 import. `@ai-jakdang/config`에서 `env`를 import.
- [x] 1.2 `ensureBotUser(persona: PersonaSeed, db: Database): Promise<string>` 함수 구현 (반환값: `users.id`).
...
- [x] 2.1 `seedBotSettings(db)` 함수 구현 — `bot_settings` 테이블에 기본값 8개 upsert.
  ```
...
- [x] 3.1 `PersonaSeed` 타입 정의 (스크립트 내부). 아래 8인 데이터를 상수로 선언.
- [x] 3.2 각 페르소나에 대해 아래 순서로 DB 삽입 (1인 = 1 트랜잭션):
...
- [x] 3.3 `seedBotPersonas(db)` 함수로 위 로직을 묶어 8인 순회 실행.
- [x] 3.4 `main()` 함수에서 `seedBotSettings(db)` → `seedBotPersonas(db)` 순서로 호출 후 `closeDb()`.
...
- [x] 4.1 **공통 헬퍼 생성**: `apps/api/src/services/bot/settings.ts`
  ```typescript
...
- [x] 4.2 **`apps/api/src/routes/v1/gamification/gamification.service.ts`의 `getRanking()` 수정**
...
- [x] 4.3 **`apps/api/src/routes/admin/dashboard/kpi.ts` 수정**
...
- [x] 4.4 **`apps/api/src/routes/admin/analytics/overview.ts` 수정**
  - `usersRows` 쿼리(날짜별 신규 가입자)에 `excludeBots=true` 시 `isBot=false` 조건 추가.

- [x] 4.5 **`apps/api/src/routes/admin/members/service.ts` 수정**
...
- [x] 5.1 Story 11.1이 `schema/bot.ts`를 생성하고 `schema/index.ts`에 `export * from './bot'`를 추가했는지 확인. 미완이면 seed 스크립트에서 직접 schema import 불가.
- [x] 5.2 `scripts/seed-bots.ts` 스크립트 실행 명령 `package.json` 또는 루트 `package.json`에 추가.
  ```
  "seed:bots": "tsx scripts/seed-bots.ts"
  ```

### Task 6: 검증
- [x] 6.1 `pnpm seed:bots` 실행 — 에러 없이 완료 확인.
- [x] 6.2 재실행 시 중복 삽입 없음 확인 (멱등 검증 — 전량 [skip] 처리).
- [x] 6.3 DB 직접 쿼리로 8인 users(is_bot=true) 확인, bot_personas 8행, bot_topics 117행 확인.
- [x] 6.4 `bot_settings` 8개 키 확인.
- [x] 6.5 getRanking() 봇 제외 필터 구현 완료 (봇은 현재 포인트 미보유, 향후 포인트 적립 시에도 excludeBots=true 조건으로 랭킹에서 제외됨).
- [x] 6.6 TypeScript 컴파일 에러 없음 (`pnpm --filter @ai-jakdang/api typecheck` 및 `@ai-jakdang/database typecheck` 모두 통과).
...
- `@ai-jakdang/database`가 루트 `node_modules`에 심볼릭 링크 없음 → `scripts/seed-bots.ts`를 얇은 래퍼로, 시드 로직 전체를 `packages/database/src/seeds/bots.ts`로 이동(스토리 Dev Notes의 대안 패턴 채택)

### Completion Notes List

- Task 1: `ensureBotUser` → `packages/database/src/seeds/bots.ts` 내부 함수로 구현. nickname 기준 SELECT → 없으면 INSERT. 시드 전용(런타임용 11.4 담당).
- Task 2: `seedBotSettings` 구현. `ON CONFLICT (key) DO NOTHING` 멱등.
- Task 3: `seedBotPersonas` 구현. 8인 순회, 1인 = 1 트랜잭션. bot_persona_boards UNIQUE 제약으로 `onConflictDoNothing`, bot_activity_rhythm/bot_topics은 COUNT SELECT → 0일 때만 INSERT.
- Task 4: `getBotExcludeFromRanking` 헬퍼 신규 생성. getRanking(gamification.service.ts), usersTotal/usersTodayNew(kpi.ts), usersRows(analytics/overview.ts), members/service.ts(isBot 필드 추가) 수정 완료.
- Task 5: `schema/index.ts`에 bot export 이미 있음(Story 11.1 완료). `package.json`에 `seed:bots` 스크립트 추가.
- Task 6(검증): 1차 실행 — users 8행·bot_personas 8행·bot_topics 117행·bot_settings 8키 생성 확인. 2차 실행(멱등) — 전량 [skip] 처리, 중복 없음 확인. typecheck(@ai-jakdang/api, @ai-jakdang/database) 통과.
- **관리자 회원 목록 isBot 필드**: 기본값 false(봇 표시 포함). 응답에 `isBot: boolean` 추가해 UI 식별 가능.
- **latte2x "외주·판매 팁" 불일치**: BOARDS에 전용 키 없어 `monetization-tips` 사용. 코드 주석으로 명시.

### File List

```
scripts/seed-bots.ts                                              (신규 — 얇은 래퍼)
packages/database/src/seeds/bots.ts                              (신규 — 시드 로직 전체)
...
package.json                                                      (수정 — seed:bots 스크립트 추가)
```
