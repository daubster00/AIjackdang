...
- [x] Task 1: API 라우트 — 활동 리듬 + 담당 게시판 엔드포인트 (AC: 1, 4)
  - [x] `PATCH /api/v1/admin/bots/:id/rhythm` — `botRhythmUpdateSchema`로 검증 후 `bot_activity_rhythm` upsert (persona_id 기준)
  - [x] `PUT /api/v1/admin/bots/:id/boards` — `botPersonaBoardUpsertSchema`로 검증 후 기존 행 삭제 + 일괄 삽입 (트랜잭션, `bot_persona_boards` 전체 교체)
  - [x] `GET /api/v1/admin/bots/:id/rhythm` — 현재 리듬 + 담당 게시판 목록 응답
  - [x] `requireSuperAdmin` preHandler 적용
  - [x] `apps/api/src/routes/admin/bots/activity-config.ts`에 등록 (메인이 index.ts 배선)

- [x] Task 2: API 라우트 — 주제 풀 엔드포인트 (AC: 2, 4)
  - [x] `GET /api/v1/admin/bots/:id/topics` — `bot_topics` 전체 목록, `status`·`board` 필터 optional
  - [x] `POST /api/v1/admin/bots/:id/topics` — `botTopicCreateSchema` 검증, `persona_id` 강제 주입 후 INSERT
  - [x] `PATCH /api/v1/admin/bots/:id/topics/:topicId` — `botTopicCreateSchema.omit({personaId}).partial()` 검증 후 UPDATE
  - [x] `DELETE /api/v1/admin/bots/:id/topics/:topicId` — DELETE (본인 persona 소유 확인)
  - [x] `GET /api/v1/admin/bots/:id/auto-refill` — 조회
  - [x] `PATCH /api/v1/admin/bots/:id/auto-refill` — `bot_settings` `bot_auto_refill_topics` 키 upsert
  - [x] `requireSuperAdmin` preHandler 전체 적용

- [x] Task 3: API 라우트 — 모델 할당 엔드포인트 (AC: 3, 4)
  - [x] `GET /api/v1/admin/bots/:id/model-assignments` — 해당 persona의 `bot_model_assignments` 목록
  - [x] `PUT /api/v1/admin/bots/:id/model-assignments` — 배열 검증 후 해당 persona 행만 전체 교체 (purpose 중복 거부)
  - [x] `requireSuperAdmin` preHandler 적용

- [x] Task 4: 관리자 라우트 등록 — 미배선(의도적)
  - [x] `registerBotActivityConfigRoutes` 함수 export 완료
  - [ ] `apps/api/src/routes/admin/bots/index.ts` 배선 — 메인이 나중에 연결

- [x] Task 5: 프론트엔드 — `/bots/[id]` 탭 패널 추가 (AC: 1~4)
  - [x] `activeTab` React 상태 추가 ("character" | "activity" | "topics" | "models")
  - [x] line-tabs 탭 버튼 4개 추가 (캐릭터·프롬프트 / 활동 설정 / 주제 풀 / 모델 할당)
  - [x] 탭 패널 display 제어 (React 상태, DOM 직접 조작 없음)

- [x] Task 6: 활동 설정 탭 UI (AC: 1, 4)
  - [x] `BotActivitySection` 컴포넌트 (별도 파일 `_components/BotActivitySection.tsx`)
  - [x] `GET /api/v1/admin/bots/:id/rhythm` 호출로 초기값 로드
  - [x] 글/주·댓글/주 입력 필드
  - [x] 활동 시간대 다중 추가/삭제 UI (from-to-crossesMidnight 행)
  - [x] 활동 요일 가중치 두 입력 (주중/주말, 합계 안내)
  - [x] 담당 게시판 섹션 — BOARD_OPTIONS(BOARDS + 특수 게시판 5종), 이미 추가된 board 제외, 가중치 입력
  - [x] "활동 설정 저장" 버튼 → `PATCH rhythm` + `PUT boards` 순차 호출

- [x] Task 7: 주제 풀 탭 UI (AC: 2, 4)
  - [x] `BotTopicsSection` 컴포넌트 (`_components/BotTopicsSection.tsx`)
  - [x] `GET /api/v1/admin/bots/:id/topics` 호출로 목록 로드
  - [x] 테이블 컬럼: title_seed, board, topic_kind, status 배지, used_at, 수정·삭제 버튼
  - [x] status 배지: unused→green, used→gray, cooling→orange
  - [x] 인라인 수정: title_seed, board(커스텀 드롭다운), series_group
  - [x] 추가 폼: title_seed, board(BOARD_OPTIONS), topic_kind, series_group
  - [x] 자동 보충 ON/OFF 토글 (전역 설정 안내 문구 포함)

- [x] Task 8: 모델 할당 탭 UI (AC: 3, 4)
  - [x] `BotModelSection` 컴포넌트 (`_components/BotModelSection.tsx`)
  - [x] `GET /api/v1/admin/bots/:id/model-assignments` 호출로 초기값 로드
  - [x] generation·censor 두 행 기본 표시, image 행 선택 추가/제거
  - [x] 각 행: provider 커스텀 드롭다운, model 텍스트, is_active 토글, note 텍스트
  - [x] "모델 저장" 버튼 → `PUT model-assignments`

- [x] Task 9: 타입 검사 및 빌드 검증 (AC: 4)
  - [x] `apps/api` typecheck — 신규 파일 오류 없음 (pre-existing 오류는 hold-queue-actions.ts·report.ts 한정)
  - [x] `apps/admin` typecheck — 통과 (오류 없음)
  - [x] `apps/admin` build — 통과 (`/bots/[id]` ƒ Dynamic 정상 빌드)
...
- `activity-config.ts` 초안에서 `handleError` 헬퍼의 `reply` 타입 어노테이션에 재귀 참조 오류(`TS2502`) 발생 → `FastifyReply` 명시 import로 수정.
- API typecheck 잔존 오류(`hold-queue-actions.ts`, `report.ts`) 는 git diff 확인 결과 기존 파일이며 이번 작업과 무관한 사전 오류.

### Completion Notes List

- `botActivityRhythm` 테이블에 `personaId` unique 제약이 schema 파일에 명시되지 않아, `onConflictDoUpdate` 대신 select-then-update/insert 패턴으로 구현 (런타임 안전).
- `PATCH /auto-refill` body 필드는 계약(`botSettingsPatchSchema`) 일치를 위해 `bot_auto_refill_topics` 키 사용 (`autoRefill` 아님). UI도 동일하게 맞춤.
- `registerBotActivityConfigRoutes` 는 `activity-config.ts`에 export 만 하고 `index.ts` 배선은 메인이 담당 (Task 4 의도적 미배선).
- 자동 보충 토글은 전역 bot_settings 키이므로 UI에 "전역 설정(모든 봇에 적용)" 안내 문구 추가.
- 섹션 컴포넌트들을 `apps/admin/app/bots/[id]/_components/` 하위 별도 파일로 분리.

### File List

- `apps/api/src/routes/admin/bots/activity-config.service.ts` (신규)
- `apps/api/src/routes/admin/bots/activity-config.ts` (신규 — `registerBotActivityConfigRoutes` export)
- `apps/admin/app/bots/[id]/_components/BotActivitySection.tsx` (신규)
- `apps/admin/app/bots/[id]/_components/BotTopicsSection.tsx` (신규)
- `apps/admin/app/bots/[id]/_components/BotModelSection.tsx` (신규)
- `apps/admin/app/bots/[id]/page.tsx` (수정 — 탭 구조 추가, 섹션 컴포넌트 임포트)
