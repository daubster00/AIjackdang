...
1. **Story 1.3 기존 코드 보존**: `databaseHooks.user.create.before/after`, `emailVerification`, `emailAndPassword` 설정 전부 보존. 소셜 신규 유저도 기존 훅을 타므로 닉네임·defaultAvatarIndex·termsAgreedAt 자동 배정 중복 구현 없음.

2. **소셜 약관 자동 동의 결정**: 소셜 가입 시 별도 약관 동의 페이지 없이, Story 1.3가 이미 구현한 `databaseHooks.user.create.before` 훅에서 `termsAgreedAt = new Date()` + `termsVersion = "2026-06"` 자동 주입으로 처리. 소셜 가입 UI에 AgreementPanel 체크박스만 표시(읽기 전용 안내). 이 결정은 ADR-0002 §카카오 정책 참고.

3. **카카오 provider 구성**: `KAKAO_ENABLED=true` AND `KAKAO_REST_API_KEY` AND `KAKAO_CLIENT_SECRET` 세 조건 모두 충족 시에만 Better Auth socialProviders에 kakao 추가. 비즈앱 검수 완료 전 기본 비활성.

4. **accountLinking trustedProviders**: `["google", "naver", "kakao"]` 로 확장. Better Auth가 verified 이메일 기준으로만 자동 연결하므로 미검증 이메일 linking 위험 없음.

5. **소셜 버튼 onClick**: Better Auth 소셜 시작 엔드포인트 `/api/v1/auth/sign-in/social/{provider}?callbackURL={encodedOrigin+redirectTo}` 로 `window.location.href` 리다이렉트. Next.js `/api/v1/*` 와일드카드 rewrite가 API 서버(4003)로 프록시.

6. **카카오 버튼 disabled UI**: `NEXT_PUBLIC_KAKAO_ENABLED !== "true"` 시 `disabled`, `title="카카오 로그인 준비 중"`, `opacity:0.4`, `cursor:not-allowed`. 버튼 구조·SVG·클래스명·텍스트 완전 보존.

7. **편차**: 스토리 Task 6의 `nickname.service.ts` 신규 생성은 필요 없음 — 기존 `assignUniqueNickname` 함수가 `user-auth.ts` 내에 이미 구현되어 있고 모든 신규 유저(소셜 포함)에 적용됨. 별도 서비스 파일 미생성.

8. **통합 테스트**: OAuth 실연동이 필요한 e2e는 이 단계에서 작성 불가(실제 callback URL + OAuth 인가 서버 필요). 유닛 레벨 mock 테스트 없음 — 기능 자체는 Better Auth 내장 핸들러 위임.

### File List
- `packages/config/src/env.ts` — `KAKAO_ENABLED` boolean(default false) 추가
- `apps/api/src/auth/user-auth.ts` — socialProviders kakao 활성화(env 조건부) + trustedProviders kakao 추가
- `apps/web/app/login/LoginForm.tsx` — 소셜 버튼 onClick 연결 + KAKAO_ENABLED 조건부 disabled
- `apps/web/app/signup/SignupForm.tsx` — SocialSignup 버튼 onClick 연결 + KAKAO_ENABLED 조건부 disabled
- `.env.example` — `KAKAO_ENABLED=false` + `NEXT_PUBLIC_KAKAO_ENABLED=false` 추가
