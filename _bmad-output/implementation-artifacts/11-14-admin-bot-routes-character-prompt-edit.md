...
- [x] Task 1: API 서비스 레이어 구현 (AC: #2, #3, #4, #5)
  - [x] `apps/api/src/routes/admin/bots/service.ts` 신규 생성
  - [x] `listBots(query)` 구현: `bot_personas`(봇 페르소나 테이블)을 `LEFT JOIN bot_activity_log`로 집계 — `lastActiveAt`(`MAX(created_at)`), `postCount`(`COUNT(event_type='post.published')`), `commentCount`(`COUNT(event_type='comment.published')`). Drizzle `getDb()` + `eq`, `ilike`, `max`, `sql` 활용.
  - [x] `getBot(id)` 구현: `bot_personas` 단건 조회 + 동일 집계 join. 미존재 시 `throw Object.assign(new Error("봇을 찾을 수 없습니다."), { code: "NOT_FOUND" })`.
  - [x] `updateBot(id, data)` 구현: partial 업데이트, 미존재 시 NOT_FOUND throw.
  - [x] `toggleBot(id)` 구현: `isActive` 반전, `returning()` 활용.
  - [x] `BotPersonaUpdate`를 `@ai-jakdang/contracts`에서 import. 로컬 타입(AdminBotListQuery 등)은 contracts에 정확한 이름 미존재로 임시 정의 + TODO 주석 표기.

- [x] Task 2: API 라우트 등록 (AC: #1, #9)
  - [x] `apps/api/src/routes/admin/bots/index.ts` 신규 생성
  - [x] `registerAdminBotsRoutes(app: FastifyInstance)` 함수 export (내부적으로 `registerBotPersonaRoutes` 호출, 후속 스토리 확장 진입점 주석 포함)
  - [x] `GET /admin/bots` 구현: 로컬 `adminBotListQuerySchema` safeParse → `listBots()` 호출
  - [x] `GET /admin/bots/:id` 구현: `getBot(id)` 호출, NOT_FOUND → 404
  - [x] `PATCH /admin/bots/:id/toggle` — 경로 충돌 방지: `/:id` 앞에 등록 (ads 패턴 동일)
  - [x] `PATCH /admin/bots/:id` 구현: `botPersonaUpdateSchema.safeParse(request.body)` → `updateBot()` 호출
  - [x] 모든 라우트에 `{ preHandler: [requireSuperAdmin] }` 적용 확인
  - [x] `apps/api/src/routes/admin/index.ts`에 `registerAdminBotsRoutes` import·등록 추가

- [x] Task 3: `/bots` 목록 페이지 구현 (AC: #6)
  - [x] `apps/admin/app/bots/page.tsx` 신규 생성
  - [x] `BotListContent` (내부 컴포넌트) + `AdminBotsPage` (Suspense 래핑) 분리
  - [x] `useSearchParams`로 `status`, `q`, `page` 쿼리 파라미터 읽기
  - [x] `fetchBots()`: `fetch(\`\${API_BASE_URL}/api/v1/admin/bots?...\`, { credentials: "include" })`
  - [x] 상태 필터 — `Select` 컴포넌트 (options: 전체/활성/비활성). native `<select>` 금지
  - [x] 테이블 행마다 토글 버튼: 클릭 → `PATCH /api/v1/admin/bots/:id/toggle` → 로컬 state 갱신 + 토스트(화면 중앙)
  - [x] `<Link href={\`/bots/\${bot.id}\`}>상세보기</Link>` 링크 포함
  - [x] 페이지네이션 (members 패턴 동일)

- [x] Task 4: `/bots/[id]` 상세·편집 페이지 구현 (AC: #7)
  - [x] `apps/admin/app/bots/[id]/page.tsx` 신규 생성
  - [x] `useParams()`로 `id` 추출 → 마운트 시 `GET /api/v1/admin/bots/:id` fetch
  - [x] 폼 state: 7개 편집 필드를 `useState` 관리
  - [x] 저장 버튼 → `PATCH /api/v1/admin/bots/:id` → 성공/실패 토스트(화면 중앙)
  - [x] `persona_prompt`(사전 프롬프트) textarea rows={12} 확보
  - [x] 404 응답 시 "존재하지 않는 봇입니다." 안내 문구 표시
  - [x] `<Link href="/bots">← 목록으로</Link>` 뒤로가기 링크

- [x] Task 5: AdminShell 네비게이션 추가 (AC: #8)
  - [x] `apps/admin/components/layout/AdminShell.tsx` 수정
  - [x] `NAV_GROUPS` 내 `"Engagement"` 그룹에 `{ key: "bots", href: "/bots", icon: "ri-robot-line", label: "활동 봇" }` 추가
  - [x] `SUPER_ADMIN_ONLY_KEYS` Set에 `"bots"` 추가

- [x] Task 6: TypeScript 타입 검사 통과 (AC: #10)
  - [x] `pnpm --filter @ai-jakdang/api typecheck` 통과 (오류 없음)
  - [x] `pnpm --filter @ai-jakdang/admin typecheck` 통과 (오류 없음)
  - [x] `pnpm --filter @ai-jakdang/admin build` 통과 (RSC 경계 위반 없음, /bots·/bots/[id] 빌드 목록 확인)
  - [x] contracts에 `AdminBotListQuery` 등 정확한 이름 미존재 → 로컬 임시 타입 정의 + TODO 주석 표기
...
- contracts/src/bot.ts에 `adminBotListQuerySchema`(status: active|inactive|all)·`AdminBotListItem`·`AdminBotDetail` 타입이 없어 service.ts·route index.ts·admin 페이지에 로컬 임시 타입 정의 + TODO 주석. `BotPersonaUpdate`(= `botPersonaUpdateSchema`)는 contracts에서 정상 import.
- PATCH body 검증에 `botPersonaUpdateSchema`(contracts) 사용 — isActive·isAdminPersona 필드도 포함되지만 service.ts의 `updateBot`이 캐릭터 시트 7개 필드만 적용해 안전.
- 토스트는 스토리 지시대로 화면 중앙(`top:50%, left:50%, transform:translate(-50%,-50%)`) 적용.
- AdminShell `"Engagement"` 그룹 내에 `bots` 항목 추가 — 후속 스토리(11.15/11.16/11.19)가 children을 추가할 수 있도록 구조 주석 포함.
- API 라우트 구조: `registerAdminBotsRoutes` 내부에서 `registerBotPersonaRoutes` 호출. 후속 스토리는 `registerAdminBotsRoutes`에 새 등록 함수를 추가하면 됨(bots/index.ts 단일 수정점).

### File List

신규 생성:
- `D:/projects/AIjackdang/apps/api/src/routes/admin/bots/service.ts`
- `D:/projects/AIjackdang/apps/api/src/routes/admin/bots/index.ts`
- `D:/projects/AIjackdang/apps/admin/app/bots/page.tsx`
- `D:/projects/AIjackdang/apps/admin/app/bots/[id]/page.tsx`

수정:
- `D:/projects/AIjackdang/apps/api/src/routes/admin/index.ts`
- `D:/projects/AIjackdang/apps/admin/components/layout/AdminShell.tsx`
