...
- [ ] 1.1 `env.ts`의 `envSchema` 객체 안에 이미 `OPENAI_API_KEY`가 있으면 건너뜀. 없으면 추가:
  ```typescript
  ...
  ```
  현재 `env.ts`에는 위 키들이 없다. Story 11.6이 AI 키를 추가할 예정이므로, 작업 전에 해당 Story 완료 여부를 확인하고 중복 추가를 피한다. 이미지 전용 키(`UNSPLASH_ACCESS_KEY`, `PEXELS_API_KEY`)는 이 스토리에서 추가한다.

- [ ] 1.2 `.env.example` 파일에 이미 시딩봇 섹션 주석이 있고 두 키가 있으므로 별도 수정 불필요.

### Task 2: server-only `images/stock.ts` 구현 (AC: #1)

- [ ] 2.1 파일 생성: `packages/server-bot/src/images/stock.ts` (worker 직접 `apps/api/src/*` import 금지)

- [ ] 2.2 `pickStock(keyword: string): Promise<StockImage | null>` 구현.
  - `StockImage` 인터페이스:
    ```typescript
...
- [ ] 2.3 Unsplash download 엔드포인트 트리거: Unsplash API 이용 약관상 이미지 선택 후 `GET <downloadUrl>` (fire-and-forget)를 해야 한다. 성공 여부와 관계없이 무시.

- [ ] 2.4 env 읽기: `process.env.UNSPLASH_ACCESS_KEY`, `process.env.PEXELS_API_KEY` 대신 `env` 객체(`@ai-jakdang/config`)에서 읽는다. Task 1 완료 후 `env.UNSPLASH_ACCESS_KEY`로 접근.

### Task 3: server-only `images/generate.ts` 구현 (AC: #2)

- [ ] 3.1 파일 생성: 권장 `packages/server-bot/src/images/generate.ts`

- [ ] 3.2 `genImage(params: GenImageParams): Promise<GenImageResult | null>` 구현.
  - 입력 타입:
    ```typescript
  ...
  - **비용 기록**: `jobId`가 있으면 `bot_generation_jobs` 테이블의 `cost` jsonb 필드를 업데이트:
    ...
    ```
  - 단, `bot_generation_jobs` 테이블은 Story 11.1에서 정의. 해당 스키마가 없으면 비용 기록 건너뜀(graceful).

- [ ] 3.3 OpenAI DALL-E 비용 상수 정의:
  ```typescript
...
- [ ] 4.1 파일 생성: 권장 `packages/server-bot/src/images/strategy.ts`
...
- [ ] 4.3 `decideImageStrategy(persona: PersonaContext, board: string, postKind: PostKind): ImageStrategy` 구현. 우선순위 순서:
  ```
  ...
  - [ ] 4.4 단위 테스트 파일 생성: 권장 `packages/server-bot/src/images/strategy.test.ts`. 주요 케이스:
...
- [ ] 5.1 파일 생성: 권장 `packages/server-bot/src/images/tiptap.ts`

- [ ] 5.2 `prependImageToTiptapDoc(doc: Record<string, unknown>, imageUrl: string, altText?: string): Record<string, unknown>` 구현:
  ```typescript
...
- [ ] 5.3 `extractFirstImageUrl`의 기존 동작 확인: `apps/api/src/lib/extract-first-image.ts`는 `node.type === 'image'` + `node.attrs?.src` (string) 인 첫 노드를 탐색한다. `prependImageToTiptapDoc` 결과물이 이 조건을 만족하는지 확인.

### Task 6: server-only `images/index.ts` 통합 함수 구현 (AC: #4)

- [ ] 6.1 파일 생성: 권장 `packages/server-bot/src/images/index.ts`

- [ ] 6.2 외부 URL에서 이미지 데이터 다운로드 헬퍼:
  ```typescript
...
- [ ] 6.4 `fetchBotImage(params: FetchBotImageParams): Promise<FetchBotImageResult>` 구현:
  ```
  ...
  - [ ] 6.5 `uploadImage` import 확인: 기존 스토리지 업로드 서비스를 server-only 경계에서 안전하게 호출할 수 있도록 래핑한다. 임시 API 내부 경로를 상대 import하지 말고, 필요한 경우 업로드 helper도 server-only 공용 경계로 re-export한다.

### Task 7: `copyright_risk` 보류 큐 연결 문서화 (AC: #4)

- [ ] 7.1 `isMeme: true` 케이스의 실제 `bot_hold_queue` 적재는 **11.9 글 생성 파이프라인** 담당임을 코드 주석으로 명시. 이 스토리는 플래그(`isMeme`)만 반환한다. `bot_hold_queue` 테이블은 Story 11.1에서 정의.

- [ ] 7.2 Dev Notes에 보류 큐 적재 인터페이스를 문서화: 11.9 구현 시 참고.

### Task 8: 검증

- [ ] 8.1 `pnpm -F @ai-jakdang/api typecheck` 에러 없음.
- [ ] 8.2 `strategy.test.ts` 단위 테스트 통과 (`pnpm -F @ai-jakdang/api test -- images/strategy`).
- [ ] 8.3 로컬 개발 환경에서 `UNSPLASH_ACCESS_KEY`를 실제 키로 설정 후 `pickStock('automation workflow')` 호출 시 이미지 URL이 반환되는지 확인 (수동 스크립트 또는 테스트).
- [ ] 8.4 `prependImageToTiptapDoc` 결과에 대해 `extractFirstImageUrl`을 호출하면 삽입한 URL이 반환되는지 확인.
- [ ] 8.5 `UNSPLASH_ACCESS_KEY`, `PEXELS_API_KEY` 모두 미설정 시 `pickStock()` 호출 → 에러 throw 없이 `null` 반환 확인.
...
```
packages/server-bot/src/images/stock.ts           (권장 신규)
packages/server-bot/src/images/generate.ts        (권장 신규)
packages/server-bot/src/images/strategy.ts        (권장 신규)
packages/server-bot/src/images/tiptap.ts          (권장 신규)
packages/server-bot/src/images/index.ts           (권장 신규)
packages/server-bot/src/images/strategy.test.ts   (권장 신규)
packages/config/src/env.ts                 (수정: UNSPLASH_ACCESS_KEY, PEXELS_API_KEY 추가)
```
