...
- Task 1 (라우팅 감사): BOARDS 상수 12개 항목 전체 확인. vibe-coding/automation/monetization 계열은 `(content)/[category]/[board]/page.tsx` 동적 라우트로 처리. notice는 `/notice/page.tsx` 독립 라우트. talk/gigs/ai-creation/ai-products는 `/lounge/` 하위 별도 정적 라우트. BOARDS.urlPath와 실제 파일 라우트 불일치 있음(talk → urlPath=/talk, 실 라우트=/lounge/talk) — 기존 레거시 구조 유지, 상세 라우팅 감사 테이블은 최종 보고에 포함.
- Task 2 (H1 감사): `BoardHero` 컴포넌트가 `<h1>`을 emit하고, board list page도 `<h1 srOnly>` 중복 → srOnly h1 제거(list page). detail page도 `<h1>{post.title}</h1>` + BoardHero h1 충돌 → `titleAs` prop 추가하여 detail page에서 BoardHero를 h2로 강등. notice/lounge 페이지는 문제 없음.
- Task 3 (JSON-LD 테스트): `jsonld.test.ts` 신규 생성, 26 tests 모두 통과. 4개 빌더 함수 @context/@type/name|headline/itemListElement[0].position 검증.
- Task 4 (SEO 통합 테스트): `seo.test.ts` 신규 생성, 11 tests 모두 통과. buildPostMeta.description===post.summary AC #4 검증.
- Task 5 (순환 의존 검사): editor.ts/post.ts/board.ts 각각 독립 — 상호 import 없음. contracts typecheck 통과.
- Task 6 (비회원 글쓰기 게이팅): `/lounge/talk` 목록은 mock 정적 데이터로 SSR 렌더. [글쓰기] 버튼은 `<Link href="/lounge/talk/write">` 직접 연결 (auth 게이팅 없음). Epic 2 동적 라우트(`(content)/[category]/[board]`)의 WriteButton은 `LoginGatingModal` 사용 — 비회원 클릭 시 모달 표시 정상. `/lounge/talk`의 직접 링크 방식은 write 페이지 자체에 게이팅 추가 필요 — 인간 결정 사항으로 defer.
- Task 7 (gate): typecheck 10/10 통과. lint — web 4개 error 중 1개(CodeBlockCopyButton react/no-danger) 수정; 나머지 3개는 resources/[slug]/page.tsx(Epic 3/4 범위 외) 미처리. contracts auth.test.ts 2개 실패는 Epic 1 pre-existing 이슈, Epic 2 범위 외.

### File List
- NEW: `apps/web/lib/seo/jsonld.test.ts`
- NEW: `apps/web/lib/seo/seo.test.ts`
- UPDATE: `apps/web/components/board/BoardHero.tsx` — `titleAs` prop 추가, h1/h2 동적 렌더
- UPDATE: `apps/web/app/(content)/[category]/[board]/page.tsx` — 중복 srOnly h1 제거
- UPDATE: `apps/web/app/(content)/[category]/[board]/[slug]/page.tsx` — BoardHero에 `titleAs="h2"` 전달
- UPDATE: `apps/web/components/board/CodeBlockCopyButton.tsx` — 미존재 react/no-danger eslint-disable 주석 제거
- UPDATE: `_bmad-output/implementation-artifacts/2-10-routing-epic-validation.md` — Status: review
