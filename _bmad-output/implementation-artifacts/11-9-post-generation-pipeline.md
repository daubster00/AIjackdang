...
- [ ] 1.1 `packages/bot-core/package.json` 생성
  ```json
  {
...
- [ ] 1.2 `packages/bot-core/tsconfig.json` 생성
  - `extends: "../../packages/config/tsconfig.base.json"` (기존 패키지 패턴 준수)
  - `include: ["src/**/*.ts"]`

- [ ] 1.3 `packages/bot-core/src/prompt-builder.ts` — 페르소나 프롬프트 빌더 (AC: #2)
...
- [ ] 1.4 `packages/bot-core/src/censor-rules.ts` — 검열 규칙 (AC: #3)
...
- [ ] 1.5 `packages/bot-core/src/duplicate-check.ts` — 단순 유사도 (AC: #3 중복성 1차 필터)
...
- [ ] 1.6 `packages/bot-core/src/index.ts` — 배럴 export

- [ ] 1.7 `packages/bot-core/src/prompt-builder.test.ts` — vitest 단위 테스트
...
- [ ] 1.8 `packages/bot-core/src/duplicate-check.test.ts` — vitest 단위 테스트
  - 동일 문자열 → 1.0
  - 완전 다른 문자열 → 0.0
...
- [ ] 2.1 `apps/api/src/services/bot/topic.ts` 신규 생성
...
- [ ] 2.2 `markTopicUsed(db, topicId: string): Promise<void>` 구현
  - `UPDATE bot_topics SET status='used', used_at=NOW() WHERE id=topicId`
  - `wasRealtime=true`인 경우 호출하지 않음 (임시 주제는 DB에 없음)

- [ ] 2.3 `apps/api/src/services/bot/topic.test.ts` — vitest 단위 테스트
...
- [ ] 3.1 `apps/api/src/services/bot/censor.ts` 신규 생성
...
- [ ] 3.2 `apps/api/src/services/bot/censor.test.ts` — vitest 단위 테스트
...
- [ ] 4.1 `apps/api/src/services/bot/post-pipeline.ts` 신규 생성
...
- [ ] 4.3 내부 헬퍼 함수 구현 (`post-pipeline.ts` 내부 또는 별도 `_helpers.ts`)
...
- [ ] 5.1 `apps/api/src/services/bot/topic.ts`에 `refillTopicsIfNeeded(db, personaId: string): Promise<number>` 추가 (새로 생성된 주제 수 반환)
...
- [ ] 5.2 `runPostPipeline` 마지막 단계에 `refillTopicsIfNeeded` 호출 추가
...
- [ ] 6.1 `packages/bot-core/src/prompt-builder.ts`의 `buildPostUserPrompt`에서 `postKind='guide'` 분기 구체화
...
- [ ] 6.2 `runPostPipeline`에서 관리자 연재 모드 로직 점검
...
- [ ] 6.3 공지사항 가드 이중 차단 확인
  - Task 4.2 Step 0의 `board === 'notices'` 가드로 파이프라인 진입 전 차단
  - 11.4 `createPostAsBot` 내부에도 동일 가드 존재 (2중 방어 확인)

### Task 7: 통합 테스트 (AC: #1~6)

- [ ] 7.1 `apps/api/src/services/bot/post-pipeline.test.ts` — vitest 통합 테스트 (DB mock 방식, 기존 `points.service.test.ts` 패턴 준수)
...
- [ ] 8.1 `packages/bot-core/src/index.ts` 배럴 export 완성
- [ ] 8.2 `apps/api/src/services/bot/index.ts` 업데이트 (`export * from './topic.js'`, `export * from './censor.js'`, `export * from './post-pipeline.js'`)
- [ ] 8.3 `pnpm typecheck --filter @ai-jakdang/bot-core` 통과
- [ ] 8.4 `pnpm typecheck --filter apps/api` 통과
- [ ] 8.5 `pnpm test --filter @ai-jakdang/bot-core` 통과
- [ ] 8.6 `pnpm test --filter apps/api` 통과 (기존 테스트 회귀 없음)
...
export async function groundTopic(topic: string, intensity: 'full' | 'light' | 'none'): Promise<FactSummary>
```

- `intensity='none'`이면 검색 없이 빈 `FactSummary` 반환 (잡담·밈)
- 검색 결과는 `<untrusted_user_content>` 블록으로 감싸 AI 요약기에 전달 (인젝션 방어). [Source: docs/seeding-bot/ARCHITECTURE.md §5]
