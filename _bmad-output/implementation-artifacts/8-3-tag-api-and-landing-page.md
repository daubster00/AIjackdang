...
- `packages/contracts` subpath exports: `"./tag": "./src/tag.ts"` 추가로 index.ts 수정 없이 하위 경로 접근 가능하게 처리
- `tags` DB 스키마 확인: `usage_count` 컬럼 없음 → popular 쿼리에서 taggable 테이블 COUNT 집계로 대체
- `taggable` (단수형) 테이블명 확인: 스토리의 `taggables`(복수)가 아닌 `taggable`(단수)이 실제 스키마
- Pagination 컴포넌트가 `onPageChange` 콜백 기반 → SSR 서버 컴포넌트에서 `<Link>` 기반 직접 구현

### Completion Notes List
- `packages/contracts/src/index.ts`는 건드리지 않음 (오케스트레이터 처리 예정)
- 대신 `packages/contracts/package.json`에 `"./tag": "./src/tag.ts"` subpath export 추가
- `apps/api/src/routes/v1/index.ts`는 건드리지 않음 (오케스트레이터 처리 예정)
- tagContent 라우트 파일을 `tagContent.ts`로 별도 생성; 등록 라인은 오케스트레이터가 추가
- 기존 API 오류(linkPreviews, popularPostsResponseSchema 등)는 8.3과 무관한 pre-existing 오류
- web 타입체크 완전 통과

### File List
- NEW: `packages/contracts/src/tag.ts`
- NEW: `apps/api/src/routes/v1/tagContent.ts`
- NEW: `apps/api/src/services/tagService.ts`
- UPDATE: `apps/web/app/tags/[tag]/page.tsx`
- UPDATE: `apps/web/app/tags/[tag]/tags.module.css` (탭·정렬·페이지버튼 CSS 추가)
- UPDATE: `packages/contracts/package.json` (subpath export 추가)
