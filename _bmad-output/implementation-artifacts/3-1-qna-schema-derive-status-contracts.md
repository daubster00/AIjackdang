...
- [ ] Task 1: DB 스키마 작성 (AC: #1, #2, #3, #4) [NEW]
  - [ ] `packages/database/src/schema/qna.ts` 생성: `pgEnum('question_status', ['draft','published','hidden','deleted'])`, `pgEnum('answer_status', ['published','hidden','deleted'])`, `answers` 테이블, `questions` 테이블(helpful_answer_id FK는 `answers` 선 생성 후 참조) 순서 엄수
  - [ ] `packages/database/src/schema/index.ts`에 `export * from './qna'` 추가 [UPDATE]
  - [ ] `taggable` 테이블은 Epic 2 소유이므로 주석으로만 명시. 실제 FK 없이 설계적으로만 표현.
  - [ ] `drizzle-kit generate` 실행 후 마이그레이션 파일 커밋 전 단독 소유권 확인
  - [ ] `drizzle-kit migrate` 실행 — DB healthy 상태에서 테스트
  - [ ] `QuestionRow`, `NewQuestionRow`, `AnswerRow`, `NewAnswerRow` 타입 export

- [ ] Task 2: `packages/core/src/qna.ts` 업데이트 (AC: #5) [UPDATE]
  - [ ] 기존 파일 읽기: 현행 `deriveQuestionStatus(state: QuestionState): QuestionStatus` + `QUESTION_STATUS_LABEL` 존재 확인
  - [ ] **현행 함수 시그니처 유지** — 호출자가 이미 '삭제된 답변 제외 후 카운트'를 전달해야 함을 JSDoc 주석으로 명시
  - [ ] `packages/core/src/qna.test.ts` [UPDATE]: ④번 케이스(삭제 답변 제외 후 0 → 'waiting') 테스트 추가. 기존 3개 테스트는 그대로 보존.

- [ ] Task 3: `packages/contracts/src/qna.ts` 신규 생성 (AC: #6) [NEW]
  - [ ] `createQuestionSchema`: `{ title: z.string().trim().min(2).max(300), contentJson: z.record(z.string(), z.unknown()), tags: z.array(z.string().trim().min(1).max(30)).max(10).default([]) }`
  - [ ] `createAnswerSchema`: `{ questionId: z.string().uuid(), contentJson: z.record(z.string(), z.unknown()) }`
  - [ ] `updateQuestionSchema`: `createQuestionSchema.partial()`
  - [ ] `updateQuestionStatusSchema`: `{ isResolved: z.boolean() }`
  - [ ] `setHelpfulAnswerSchema`: `{ answerId: z.string().uuid().nullable() }`
  - [ ] `questionListQuerySchema`: `paginationQuerySchema.extend({ status: z.enum(['all','waiting','answered','resolved','popular']).default('all'), sort: z.enum(['latest','popular']).default('latest') })`
  - [ ] `questionDetailResponseSchema`, `answerResponseSchema`: 응답 타입 정의 (id, title, status derived, user 정보, 날짜, 태그, answerCount 포함)
  - [ ] 추론 타입(CreateQuestionInput, CreateAnswerInput 등) export

- [ ] Task 4: contracts 인덱스 등록 + 전체 typecheck (AC: #7) [UPDATE]
  - [ ] `packages/contracts/src/index.ts`에 `export * from './qna'` 추가
  - [ ] `pnpm typecheck` 전 워크스페이스 통과 확인

- [ ] Task 5: Vitest 실행 (AC: #5, #8)
  - [ ] `pnpm test` — `packages/core/src/qna.test.ts` 4개 케이스 전부 green 확인
