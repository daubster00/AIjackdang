...
- [ ] 1.1 `QUEUE_NAMES` 객체 끝에 `bot` 항목 추가. 다른 항목 변경 금지.
  ```typescript
...
- [ ] 2.1 파일 생성. 기존 `rankingProcessor` 디스패처(`index.ts` §6.3/6.5 블록의 `switch` 패턴) 재사용.
...
- [ ] 2.2 위 import 대상(`./daily-plan.processor.js`, `./write.processor.js`, `./comment.processor.js`)이 각각 Story 11.11, 11.9, 11.10에서 생성되었는지 확인한다. **없으면** 해당 파일의 빈 stub을 임시 생성한다(Dev Notes §선행 의존성 참고). stub 형식:
  ```typescript
  import type { Job } from 'bullmq';
...
- [ ] 3.1 파일 생성. Story 11.17에서 실 구현 예정. 이 스토리에서는 로그만 출력.
  ```typescript
  /**
...
- [ ] 4.1 파일 생성. 주제 자동 보충 로직은 Story 11.9(글 생성 파이프라인)에서 구현한 함수에 연결 예정.
  ```typescript
  /**
...
- [ ] 5.1 파일 생성. **`ranking.cron.ts` 패턴 재사용** (`apps/worker/src/schedules/ranking.cron.ts` 참고).
...
- [ ] 6.1 기존 `// ── [9.10] content.cleanup cron END ────` 블록 직후에 봇 블록을 추가한다. 기존 블록 일체 변경 금지.

- [ ] 6.2 아래 블록을 `index.ts` 끝부분(SIGINT/SIGTERM 핸들러 **이전**)에 삽입:
...
- [ ] 6.3 `SIGINT`/`SIGTERM` 핸들러 내 `shutdown()` 함수는 이미 `workers.map((w) => w.close())`를 사용하므로 수정 불필요. 봇 워커는 6.2에서 `workers.push(botWorker)` 했으므로 자동 포함된다.

### Task 7: `apps/api/src/lib/queues.ts` 수정 — `getBotQueue()` 추가 (AC: #1)

봇 잡 수동 enqueue(관리자 UI·11.14~11.16)·일일 계획에서 write/comment 잡 발행에 필요.

- [ ] 7.1 기존 `getRankingQueue()` 패턴(지연 초기화 싱글톤) 그대로 복사해 `getBotQueue()` 추가:
...
- [ ] 8.1 `apps/worker/src/processors/bot/bot.processor.test.ts` 생성. 기존 `rankingCompute.processor.test.ts`의 Vitest 구조 참고.
...
- [ ] 8.2 `pnpm -F @ai-jakdang/worker test` 기존 테스트 회귀 없음 확인.

### Task 9: TypeScript 검증

- [ ] 9.1 `pnpm -F @ai-jakdang/worker typecheck` — 컴파일 에러 없음 확인.
- [ ] 9.2 `pnpm -F @ai-jakdang/api typecheck` — `queues.ts` 수정 후 에러 없음 확인.
