...
- [x] 1.1 파일 생성: `packages/server-bot/src/gates.ts`. worker가 `apps/api/src/*`를 직접 import하는 구조는 금지한다.

- [x] 1.2 `getBotSetting<T>(key: string): Promise<T | null>`는 공용 bot settings helper에서 import.
...
- [x] 1.3 `isCostLimitReached(db?): Promise<boolean>` 구현.
...
- [x] 1.4 `checkBotGates(jobKind: 'write' | 'comment' | 'plan' | 'report' | string): Promise<GateResult>` 구현.
  단계별 순서:
...
- [x] 1.5 `logBotSkip(personaId: string | null, reason: string, jobKind: string): Promise<void>` 유틸 구현.
...
- [x] 2.1 **`apps/worker/src/processors/bot/write.processor.ts`** 상단(processor 함수 진입 즉시): _(스코프 경계: write.processor.ts는 11.13 담당. 이 스토리에서는 comment.processor만 배선 — 2.2 완료)_
  ```typescript
  // 게이트 체크 (AC: #1~#4)
...
- [x] 2.2 **`apps/worker/src/processors/bot/comment.processor.ts`** 동일 패턴 적용 (`jobKind='comment'`).

- [x] 2.3 **`apps/worker/src/processors/bot/daily-plan.processor.ts`** (`bot.daily-plan` 잡): 게이트 중 **킬 스위치만** 확인한다(계획 생성은 글 수 상한·비용 상한에 걸릴 필요 없음). `checkBotGates('plan')` 호출 시 내부적으로 step 2~4(상한 체크)를 'plan' 종류에 대해 건너뜀. _(스코프 경계: daily-plan.processor.ts는 11.11 담당. checkBotGates 내부적으로 'plan' jobKind는 킬 스위치만 확인하도록 구현 완료)_

### Task 3: `holdForObservation` 유틸 구현 (AC: #4)

- [x] 3.1 `apps/api/src/services/bot/holdQueue.ts`(또는 기존 파일에 추가) 내에 `holdForObservation(jobId: string, draftContent: unknown, reason: string): Promise<void>` 구현.
...
- [x] 4.1 `gates.ts` 내에 `getDailyPublishedCount(eventType: 'post.published' | 'comment.published'): Promise<number>` 구현.
...
- [x] 5.1 Story 11.5가 이미 `apps/api/src/services/bot/settings.ts`에 `getBotExcludeFromRanking`을 구현했다면, 공용 `getBotSetting('bot_exclude_from_ranking')` 내부 호출 형태로 리팩터링한다.
- [x] 5.2 Story 11.16의 settings API와 중복되지 않도록 `apps/api/src/lib/botSettings.ts`를 API용 단일 유틸로 사용한다. worker/server-only 경계에서는 동일 정책의 helper를 별도 노출한다.

### Task 6: 단위 테스트 (AC: #1~#7)

- [x] 6.1 `apps/api/src/services/bot/gates.test.ts` 신규.
...
- [x] 7.1 `pnpm -F @ai-jakdang/api typecheck` — 타입 에러 없음. (server-bot, api, worker 3패키지 모두 통과)
- [ ] 7.2 `bot_settings`에 `bot_master_enabled=false` 세팅 후 `bot.write` 잡 enqueue → `bot_activity_log`에 `skipped/master_disabled` 항목 확인. _(11.13 워커 등록 후 e2e 검증 예정)_
- [ ] 7.3 `bot_daily_post_limit=0` (0으로 낮춤) 설정 후 `bot.write` 잡 → skip + `daily_post_limit_exceeded` 로그 확인. _(11.13 워커 등록 후 e2e 검증 예정)_
- [ ] 7.4 `bot_observation_mode=true` 설정 후 `bot.write` 잡 → `bot_hold_queue`에 `reason='observation_mode'` 레코드 생성 확인. `bot_generation_jobs.status='held'` 확인. _(11.13 워커 등록 후 e2e 검증 예정)_
...
1. **TypeScript 오류 (TS2558)**: `vi.fn<[string], Promise<unknown>>()` 구문이 Vitest 3.x에서 타입 인수 0~1개 예상 오류 발생 → `vi.fn<(key: string) => Promise<unknown>>()` 단일 함수 타입 형태로 수정
2. **worker `@ai-jakdang/server-bot` 미해결**: worker의 node_modules에 server-bot 없음 (API 노드모듈에만 존재) → `pnpm install` 금지 제약으로 `apps/worker/tsconfig.json`에 `paths` alias 추가로 해결 (`"@ai-jakdang/server-bot/*": ["../../packages/server-bot/src/*"]`)
3. **11.6 비용 상한 충돌**: 11.6의 `checkDailyCostLimit`은 UTC 자정 기준 + throw 방식 → 이 스토리의 `isCostLimitReached`는 KST 자정 기준 + boolean 반환으로 독립 구현 (11.6 ai/index.ts 수정 금지 제약)
4. **`logBotSkip(null)` DB NOT NULL 제약**: `bot_activity_log.persona_id`는 NOT NULL → personaId가 null이면 DB INSERT 스킵, console.warn만 출력

### Completion Notes List

- **gates.ts 위치**: Story 권장 경로인 `packages/server-bot/src/gates.ts` 사용 (스토리 파일 목록의 `apps/api/src/services/bot/gates.ts`는 권장 경로 아님, 미생성)
- **worker 경계 준수**: worker는 `apps/api/src/*` 직접 import 없음. `@ai-jakdang/server-bot/gates` paths alias로 해결
- **write.processor.ts·daily-plan.processor.ts 미배선**: 각각 11.13/11.11 담당 스코프. 이 스토리에서는 comment.processor.ts만 게이트 배선. checkBotGates 내부 'plan'/'report' 분기는 완전 구현됨
- **11.6 비용 중복 방지**: `isCostLimitReached` 독립 구현 (11.6 ai/index.ts 수정 금지). 11.13 이후 11.6 개발자와 중복 제거 협의 권장
- **사전존재 테스트 실패 3건**: dashboard.test.ts(1건)·ranking.service.test.ts(6건)·messages/routes.test.ts(2건) — 이 스토리 변경과 무관한 선행 실패. ranking 실패는 innerJoin 목 미설정 문제로 getBotExcludeFromRanking db 파라미터 제거 전/후 동일하게 실패

### File List

```
packages/server-bot/src/gates.ts              (신규 — checkBotGates·isCostLimitReached·logBotSkip·getDailyPublishedCount·getKstDayStart)
packages/server-bot/src/botSettings.ts        (신규 — DB 직접 getBotSetting, worker/server-only용)
packages/server-bot/package.json              (수정 — ./gates·./botSettings exports 추가)
apps/api/src/lib/botSettings.ts               (신규 — Redis 캐시 getBotSetting·setBotSetting·getAllBotSettings·invalidateBotSetting, API 프로세스용)
apps/api/src/services/bot/holdQueue.ts        (신규 — holdForObservation)
apps/api/src/services/bot/gates.test.ts       (신규 — 17개 단위 테스트 전부 통과)
apps/api/src/services/bot/settings.ts         (수정 — getBotExcludeFromRanking를 getBotSetting 재사용으로 리팩터링, db 파라미터 제거)
apps/api/src/services/bot/index.ts            (수정 — holdQueue.ts re-export 추가)
apps/api/src/routes/v1/gamification/gamification.service.ts  (수정 — getBotExcludeFromRanking() 호출 db 인수 제거)
apps/api/src/routes/admin/dashboard/kpi.ts   (수정 — getBotExcludeFromRanking() 호출 db 인수 제거)
apps/api/src/routes/admin/analytics/overview.ts (수정 — getBotExcludeFromRanking() 호출 db 인수 제거)
apps/worker/src/processors/bot/comment.processor.ts  (수정 — checkBotGates·logBotSkip 게이트 배선)
apps/worker/tsconfig.json                     (수정 — @ai-jakdang/server-bot/* paths alias 추가)
```
...
| 패키지 | typecheck | 비고 |
|---|---|---|
| `@ai-jakdang/server-bot` | PASS | |
| `@ai-jakdang/api` | PASS | |
| `@ai-jakdang/worker` | PASS | |

| 테스트 파일 | 결과 | 비고 |
|---|---|---|
| `gates.test.ts` | 17/17 PASS | checkBotGates 12건 + getKstDayStart 2건 + logBotSkip 3건 |
| `dashboard.test.ts` | 1 FAIL | 선행 실패 (이 스토리 무관) |
| `ranking.service.test.ts` | 6 FAIL | 선행 실패 — innerJoin 목 미설정 (이 스토리 무관) |
| `messages/routes.test.ts` | 2 FAIL | 선행 실패 (이 스토리 무관) |
