...
function buildResourceCurationUserPrompt(
  ...
  // 자료의 공식 README/문서 원문(있으면). 설치 명령·기능을 지어내지 않고 이 원문에서 뽑게 한다.
  const readmeBlock = rc.readme
    ? `<자료_원문_문서>\n아래는 이 자료의 공식 README/문서에서 가져온 실제 내용입니다. 설치 명령·설정 블록·기능 설명은 반드시 이 원문에서 확인된 것만 쓰고, 여기 없는 명령·옵션은 지어내지 마세요.\n---\n${rc.readme}\n---\n</자료_원문_문서>`
    : "";
  ...
  const rules = [
    ...
    "이 글은 커뮤니티 '실전자료' 게시판에, 실제로 존재하고 많은 사람이 쓰는 자료를 발굴해 '당장 써먹게' 소개하는 글입니다.",
    "당신이 자료를 새로 지어내거나 창작하는 글이 아닙니다. 위 <소개 대상>의 실제 자료를, 초보자도 이해하고 바로 따라 할 수 있게 상세하고 쉽고 일목요연하게 소개하세요.",
    "",
    "아래 구성을 순서대로, 소제목(##)으로 나눠 작성하세요(각 소제목은 짧고 명확하게):",
    "1. 도입(2~3문장): 이 자료가 무엇이고, 어떤 문제·상황에서 쓰는지. 어려운 말 없이 쉽게.",
    "2. '왜 좋은가 / 왜 유명한가': 핵심 가치와 인기 근거(공식·스타 수·자주 언급됨 등)를 근거에 기반해 짧은 항목들로.",
    "3. '무엇을 할 수 있나 / 무엇이 들어있나': 주요 기능·구성을 <자료_원문_문서>에서 확인된 선에서 구체적으로 항목화.",
    "4. '설치와 사용법': 실제 설치 명령·설정(설정 파일 블록·연결 명령 등)을 반드시 ```코드블록```으로 정확히 제시하세요. 원문 문서에 있는 명령을 그대로 쓰고, 없으면 '공식 문서 참고'로 안내하되 지어내지 마세요. 이어서 실제로 어떻게 쓰는지 한두 예시.",
    "5. '주의할 점': 함정·한계·비용·보안 등 1~3개 항목.",
    `6. '출처와 다운로드': 본문에 실제 링크(${rc.sourceUrl})를 노출하고, 하단 첨부파일로 받을 수 있다고 안내하세요(첨부는 시스템이 붙입니다).`,
    "",
    "품질 기준(매우 중요): '~로 소개되는 자료입니다', '확인된 범위에서는', '자세한 건 공식 페이지에서 확인하세요' 식으로 얼버무리며 끝내지 마세요. 원문 문서를 근거로, 독자가 이 글만 읽고도 설치·사용을 시작할 수 있을 만큼 구체적으로 쓰세요.",
    "금지: 원문 문서에 없는 기능·수치·명령·별점을 지어내기, 자료 전체를 통째로 베끼기(소개·요약만), 자료를 직접 만든 것처럼 쓰기. 불확실하면 \"~로 알려짐\" 형식으로 완화하세요.",
  ].join("\n");

  const format = [
    "마크다운으로 작성하세요. 소제목은 ##, 명령·설정·코드는 반드시 ```코드블록```으로 감쌉니다.",
    "출처 링크는 마크다운 링크나 URL 그대로 본문에 노출하세요.",
    "문단과 문단 사이는 빈 줄 하나로 띄웁니다. 이모지·상투적 인사말 금지. 글 제목은 쓰지 마세요(제목은 시스템이 붙입니다).",
  ].join("\n");
...
function buildCommunityCurationUserPrompt(
  ...
  const mediaWord = cc.mediaIsGif ? "움짤(gif)" : "이미지";
  const subjectBlock = [
    "<소개 대상>",
    `출처 커뮤니티: ${cc.site}`,
    `원문 제목: ${cc.originalTitle}`,
    cc.excerpt ? `원문 발췌: ${cc.excerpt}` : null,
    cc.hasMedia ? `첨부: 원문의 ${mediaWord}이(가) 이미 이 글 맨 위에 그대로 붙어 있음` : null,
    `원문 링크: ${cc.sourceUrl}`,
    "</소개 대상>",
  ]
    .filter(Boolean)
  ...
  // 미디어(짤)가 첨부됐는지에 따라 톤을 나눈다.
  // - 첨부됨: 실제 사람이 커뮤니티서 짤 하나 퍼와서 "이거 봐봐 ㅋㅋ"라고 올리는 그 느낌.
  //   원문 제목/발췌의 위트·맥락은 그대로 살리되, 표현만 우리 수다방 말투로 아주 살짝 다듬는다.
  // - 미첨부: 이미지 없이 '요즘 이런 글이 화제더라'며 텍스트로 소개.
  const rules = cc.hasMedia
    ...
    : [
        ...
        `이 글은 'AI·자동화에 관심 많은 사람들이 가볍게 떠드는 수다방'에, 요즘 ${cc.site} 같은 국내 커뮤니티에서 화제가 된 글을 발견해 소개하는 캐주얼한 글입니다.`,
        "당신이 직접 겪은 일처럼 지어내지 말고, '요즘 이런 글이 화제더라'며 소개·리액션하는 톤으로 쓰세요.",
        "",
        "구성:",
        "1. 도입: 요즘 어디(커뮤니티)에서 이런 글/이슈가 화제라고 자연스럽게 운을 뗍니다.",
        "2. 무엇이 화제인지: 원문 제목·발췌가 말하는 주제를 소개하고, 왜 사람들이 반응하는지 가볍게 곁들입니다.",
        "3. 리액션: 커뮤니티 유저처럼 짧은 감상·의견을 한두 줄.",
        `4. 출처: 본문에 원문 링크(${cc.sourceUrl})를 반드시 넣어 '원문은 여기서 볼 수 있다'고 안내합니다.`,
        "",
        "금지: 원문 본문을 통째로 베끼기, 제목·발췌에 없는 구체 사실·수치·인용을 지어내기, 이모지·기계적 상투어, 'AI'를 억지로 엮기.",
        "확신할 수 없는 내용은 '~인 듯', '~라는 글' 처럼 완곡하게 표현하세요.",
      ].join("\n");
