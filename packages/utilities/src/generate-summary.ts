/**
 * Tiptap JSON → 평문 요약 추출기 (서버 사이드 — packages/utilities)
 *
 * Story 2.7: API service 레이어에서 게시글 저장 시 summary 를 자동 생성하는 용도.
 * `apps/web/lib/seo/generate-summary.ts` 와 동일 로직을 서버(API)용으로 제공한다.
 * 패키지 경계 규칙: web lib 을 api 에서 직접 import 금지 (project-context §패키지 경계).
 *
 * 지원 노드:
 *   - 텍스트 기여: `text`, `paragraph`, `heading`, `blockquote`, `listItem`, `tableCell` 등
 *   - 공백 1개로 대체: `image`, `hardBreak`
 *   - 텍스트 기여 없음(무시): `codeBlock`, `horizontalRule`, `code`
 */
