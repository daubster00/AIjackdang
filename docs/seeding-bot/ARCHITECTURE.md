...
```
   ...
   (순수함수) │ 인젝션 필터·맥락 요약·중복 판정      │            │
              └──────────────────┬─────────────────┘            │
                                 │ 호출                          │
                 ┌───────────────▼─────────────────────────────┴──┐
                 │  apps/worker  (BullMQ)  ── 봇 잡(job) 추가       │
                 │  bot-orchestrator(daily plan) · bot-write       │
                 │  · bot-comment · bot-censor · bot-report(cron)  │
                 └──┬────────────┬──────────────┬──────────────┬──┘
                    │            │              │              │
            ┌───────▼──┐  ┌──────▼─────┐  ┌─────▼──────┐  ┌────▼──────┐
...
| `apps/worker` (수정) | 봇 워커·잡·크론 (`processors/bot/*`, `schedules/bot.cron.ts`) | 기존 BullMQ 인프라 위 |
| `apps/admin` (수정) | "활동 봇" 메뉴·페이지 | `packages/admin-design-system` 사용 |
| `apps/api/src/lib/ai/*` (신규) | AI 프로바이더 추상화 어댑터 | OpenAI/Claude/Gemini |
| `apps/api/src/lib/search/*` (신규) | 검색 어댑터 | Google/Naver |
| `apps/api/src/lib/images/*` (신규) | 이미지 엔진 | 스톡/생성 |
...
| gen_model_id | uuid FK→bot_model_assignments | 글 생성용 모델 |
| censor_model_id | uuid FK→bot_model_assignments | 검열관용 모델 |
...
### 2.6 `bot_model_assignments` (모델 할당)
- `provider`(프로바이더) enum(`openai`|`anthropic`|`google`), `model`(모델명) varchar, `purpose`(용도) enum(`generation`|`censor`|`image`), `is_active`, `note`. 봇이 참조.
...
- `job_id` FK, `reason`(보류 사유: `ambiguous`|`injection_suspect`|`copyright_risk`), `decided`(결정됨) boolean, `decision`(`approved`|`discarded`), `decided_at`, `decided_by`(admin_user_id).

### 2.9 `bot_activity_log` (활동 로그) — 사후 감시·일일 요약 원천
- `persona_id`, `event_type`(이벤트: `post.published`|`comment.published`|`held`|`blocked`|`regenerated`|`skipped`|`cost`), `ref_id`, `payload` jsonb, `created_at`.
...
> 마이그레이션은 기존 최신(0022) 다음 번호부터. drizzle `db:generate`로 캡처 후 `db:migrate`.
...
위치: `apps/api/src/services/bot/`. **기존 사용자 작성 service를 재사용**한다.
...
> **선행 리팩터링**(Epic 11): 댓글 생성 도메인 로직을 `comments/service.ts`로 추출해 봇·사용자가 공유.
> contentGuard는 현재 preHandler(`apps/api/src/middleware/contentGuard.ts`)이므로, 봇 경로에서는
> 동일 검사를 **함수로 직접 호출**(`runContentGuard(text)`)할 수 있게 추출한다.

---

## 4. AI 추상화 레이어 (`apps/api/src/lib/ai/`)
...
## 5. 검색 어댑터 (`apps/api/src/lib/search/`)
...
## 6. 이미지 엔진 (`apps/api/src/lib/images/`)
...
| 큐/잡 이름 | 트리거 | 처리 |
|---|---|---|
| `bot.daily-plan` | cron 매일 새벽 | 오늘 활동 계획 랜덤 생성 → 개별 잡 enqueue(분 단위 랜덤 delay) |
| `bot.write` | 스케줄된 시각 | 글 생성 파이프라인 1건 |
| `bot.comment` | 스케줄된 시각 | 댓글 생성 파이프라인 1건 |
| `bot.censor` | 글/댓글 후보 생성 후 | 검열관 판정(분리 잡으로 둘 수도, 인라인도 가능) |
| `bot.daily-report` | cron 매일 아침 | 어제 활동 집계 → 대시보드 데이터 + 텔레그램 푸시 |
...
Epic 16 (댓글 파이프라인)─┤
        ↓               │
Epic 17 (오케스트레이터·안전장치) ← 15·16을 굴림
        ↓
Epic 18 (관리자 대시보드) ← 11~17의 설정·운영 UI
Epic 19 (일일 리포트·텔레그램) ← 17의 로그 위
```
