...
/** GET /api/v1/me/resources 응답 — 본인 자료 목록 */
const myResourceItemSchema = z.object({
  id: z.string().uuid(),
  slug: z.string(),
  title: z.string(),
  summary: z.string(),
  resourceType: z.string(),
  status: z.string(),
  /** hidden 자료 사유 안내 (null이면 사유 없음) */
  hiddenReason: z.string().nullable(),
  createdAt: z.string(),
  updatedAt: z.string(),
});

const myResourcesQuerySchema = z.object({
  /** 콤마 구분 상태 필터 (기본: draft,published,hidden) */
...
export async function registerResourceMutateRoutes(app: FastifyInstance): Promise<void> {
  ...
  // ── GET /me/resources — 본인 자료 목록 (마이페이지 탭용) ──────────────────
  typed.get(
    "/me/resources",
    {
      preHandler: [requireAuthHook],
      schema: {
        description:
          "본인 자료 목록. 인증 필수. status 쿼리로 필터(기본: draft,published,hidden). " +
          "deleted 자료는 항상 제외. hiddenReason: hidden 자료에 사유 안내.",
        tags: ["me", "resources"],
        querystring: myResourcesQuerySchema,
        response: {
          200: z.array(myResourceItemSchema),
          401: errorResponseSchema,
        },
      },
    },
    async (request, reply) => {
      ...
      const items = await getMyResources(userId, statuses);
