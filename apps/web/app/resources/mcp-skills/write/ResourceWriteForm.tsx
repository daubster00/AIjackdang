...
/**
 * 실전자료 등록 폼 (MCP·Skills).
 * 일반 게시판 글쓰기(PostWriteForm)와 달리, 실전자료는 다운로드형 자료실이라
 * "첨부 파일 + 설명"이 핵심이다. 모든 자료 유형은 같은 등록폼을 공유한다.
 */

/** 자료 유형 — MCP·Skills 하위 게시판은 Claude Code Skill / MCP 두 유형을 받는다. */
const resourceTypes = [
  { value: "skill", label: "Claude Code Skill", icon: "magic-line", hint: "zip 권장 · SKILL.md 포함" },
  { value: "mcp", label: "MCP", icon: "plug-line", hint: "zip 권장 · 설정 JSON 포함" },
] as const;
...
const SUGGESTED_TAGS = [
  "ClaudeCode", "MCP", "Skills", "GitHub", "Notion",
  "자동화", "프롬프트", "Rules", "Cursor", "검증",
];
  ...
  return (
    <form className={styles.form} onSubmit={handleSubmit}>
      <header className={styles.formHeader}>
        ...
        <h1 className={styles.formTitle}>MCP·Skills 자료 등록</h1>
        <p className={styles.formDesc}>
          스킬·MCP 설정 파일을 올리고 설명을 적으면 다른 사람이 받아서 바로 적용할 수 있습니다.
      ...
      <div className={styles.field}>
        ...
        <input
          ...
          type="text"
          placeholder="자료 이름을 입력하세요 (예: 코드 품질 자동 리뷰 스킬)"
          value={title}
          maxLength={80}
      ...
      <div className={styles.field}>
        ...
        <div className={styles.textareaBox}>
          <textarea
            ...
            placeholder="무엇을 하는 자료인지, 어떻게 설치·적용하는지 설명해 주세요."
            value={desc}
            maxLength={MAX_DESC}
        ...
        <div className={styles.tagBox}>
          ...
          {tags.length < MAX_TAGS && (
            <input
              ...
              type="text"
              placeholder={tags.length === 0 ? "태그를 입력하고 Enter (예: ClaudeCode)" : ""}
              value={tagInput}
      ...
      <div className={styles.formActions}>
        <Link href="/resources/mcp-skills" className={styles.cancelBtn}>
          취소
        </Link>
