...
export function ReactionBar({
  ...
  useEffect(() => {
    if (!user) return;
    if (!isSelf) {
      void fetch(`/api/v1/reactions/me?targetType=${targetType}&targetId=${resourceId}`, { credentials: "include" })
    ...
    void fetch(`/api/v1/users/me/bookmarks/status?targetType=${targetType}&targetId=${resourceId}`, { credentials: "include" })
      .then((res) => (res.ok ? res.json() : null))
  ...
  async function toggleLike() {
    ...
    if (isSelf) {
      toast({ tone: "warning", title: "내 자료에는 좋아요를 누를 수 없습니다." });
      return;
    }
    ...
    try {
      if (prevLiked && prevReactionId) {
        ...
        const res = await fetch("/api/v1/reactions", { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ targetType, targetId: resourceId, reactionType: "like" }) });
        if (!res.ok) throw new Error();
  ...
  async function toggleBookmark() {
    ...
    if (bookmarked && bookmarkId) {
      ...
      try {
        const res = await fetch("/api/v1/bookmarks", { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ targetType, targetId: resourceId }) });
        if (!res.ok) throw new Error();
        const data = (await res.json()) as { id: string };
  ...
  return (
    <>
      <div className={styles.detailActions} aria-label="자료 반응">
        <button
          type="button"
          className={liked ? styles.reactionBarBtnActive : undefined}
          onClick={() => void toggleLike()}
          aria-pressed={liked}
          aria-label={isSelf ? "내 자료는 좋아요할 수 없습니다" : `좋아요 ${likeCount}개`}
        >
