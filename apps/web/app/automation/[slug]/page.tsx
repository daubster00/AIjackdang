...
import { BoardHero, AttachmentList, CodeBlockCopyButton, DeleteButton, RecentViewedTracker, ViewBeacon } from "@/components/board";
import {
  SITE_URL,
  buildPostMeta,
  buildPostUrl,
...
export default async function AutomationDetailPage({ params }: PageProps) {
  ...
  const boardUrl = boardMeta ? `${SITE_URL}${boardMeta.urlPath}` : `${SITE_URL}/automation`;
  // 상세 URL 은 쿼리스트링을 제거한 경로로 생성(하위게시판 breadcrumb URL 깨짐 방지).
  const postUrl = buildPostUrl(post.board, post.slug);
  const boardLabel = boardMeta?.label ?? "자동화 가이드";
  const boardCategory = boardMeta?.category ?? "ai-automation";
  const listUrl = boardMeta?.urlPath ?? "/automation";
  ...
  const structuredDataJsonLd = buildDiscussionJsonLd(post, boardMeta?.urlPath ?? "/automation");
  ...
  return (
    <main id="main" className={styles.page}>
      ...
      <BoardHero menu="automation" currentSub={boardLabel} />
