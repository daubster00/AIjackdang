 ...
 *
 * monetization-tips 게시판의 게시글을 API에서 SSR로 불러온다.
...
async function fetchPosts(sort: string, page: number, cookie: string): Promise<PaginatedPosts> {
  try {
    const res = await fetch(
      `${API_URL}/api/v1/posts?board=monetization-tips&sort=${sort}&page=${page}&pageSize=20`,
          ...
          {meta.totalPages > 1 && (
            <nav className={styles.pagination} aria-label="페이지 이동">
              ...
              {Array.from({ length: meta.totalPages }, (_, i) => i + 1).map((p) => (
                <Link key={p} href={p === 1 ? "/monetize" : `/monetize?page=${p}`}>
